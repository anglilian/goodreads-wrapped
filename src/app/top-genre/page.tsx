...
`;
      ...
      const { genre, isbns } = parseResponse(data.response);
...
function parseResponse(response: string): { genre: string; isbns: string[] } {
  ...
  try {
    ...
    const genre = lines[0].replace(/You read a lot about/i, "").trim();
